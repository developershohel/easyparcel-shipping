...
// add_action('woocommerce_after_shipping_rate', 'easyparcel_pickup_point', 20, 2); #use easyparcel.php ep_pickup_point_shipping function
add_action( 'wp_footer', 'easyparcel_pickup_point_script_js' );
add_action( 'wp_ajax_ep_pickup_point', 'easyparcel_set_ep_pickup_point' );
add_action( 'wp_ajax_nopriv_ep_pickup_point', 'easyparcel_set_ep_pickup_point' );
add_action( 'woocommerce_checkout_create_order', 'easyparcel_save_pickup_point_as_order_meta', 30, 1 );
add_action( 'woocommerce_admin_order_data_after_shipping_address', 'easyparcel_admin_order_display_ep_pickup_point', 30, 1 );
...
function easyparcel_pickup_point( $method, $index ) {
	extract( easyparcel_pickup_point_settings() ); // Load settings and convert them in variables
...
function easyparcel_pickup_point_script_js() {
            ...
            jQuery(function ($) {
                ...
                $(document.body).on('change', 'select#<?php echo esc_attr( $field_id ); ?>', function () {
                    ...
                    $.ajax({
                        ...
                        data: {
                            'action': 'easyparcel_pickup_point',
                            'value': value
                        },
...
function easyparcel_has_pickup_point_field() {
	$settings = easyparcel_pickup_point_settings();
...
function easyparcel_admin_order_display_ep_pickup_point( $order ) {
	extract( easyparcel_pickup_point_settings() );
...
function easyparcel_display_fields_on_order_item_totals( $total_rows, $order, $tax_display ) {
	...
	foreach ( $total_rows as $key => $values ) {
		...
		if ( $key === 'shipping' ) {
			extract( easyparcel_pickup_point_settings() );
			$ep_pickup_point = $order->get_meta( '_' . $field_id ); // Get easyparcel_pickup_point
			if ( ! empty( $ep_pickup_point ) ) {
			...
			$ep_tracking_url            = ( ! empty( $order->get_meta( '_ep_tracking_url' ) ) ) ? '<a href="' . esc_url( $order->get_meta( '_ep_tracking_url' ) ) . '" target="_blank"><u>' . esc_html( $ep_awb ) . '</u></a>' : '- '; // Get EP Tracking URL
