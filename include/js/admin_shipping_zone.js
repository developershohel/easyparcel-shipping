        ...
        var $table = $('.wc-shipping-zones'),
            ...
            ShippingZone = Backbone.Model.extend({
                ...
                save: function () {
                    if (_.size(this.changes)) {
                        $.post(ajaxurl + (ajaxurl.indexOf('?') > 0 ? '&' : '?') + 'action=easyparcel_shipping_zones_save_changes', {
            ...
            ShippingZoneView = Backbone.View.extend({
                ...
                initRow: function (rowData) {
                    ...
                    view.renderCouriers(rowData.zone_id, rowData.couriers);
                ...
                renderCouriers: function (zone_id, couriers) {
                    ...
                    if (_.size(couriers)) {
                        couriers = _.sortBy(couriers, function (method) {
                            return parseInt(method.courier_order, 10);
                        });

                        _.each(couriers, function (courier) {
                            var class_name = 'method_disabled';

                            if ('yes' === courier.enabled) {
                                class_name = 'method_enabled';
                            }

                            $method_list.append(
                                '<li class="wc-shipping-zone-method ' + class_name + '" style="display:block">' + courier.courier_display_name + ' - (' + courier.rate + ')' + '</li>'
                            );
                        });
                    } else {
                        $method_list.append('<li class="wc-shipping-zone-method">' + data.strings.no_courier_applied + '</li>');
                    }
                },
