    ...
    $(function () {
        ...
        $("select#courier_services").change(function (e) {
            if ($(e.target).find(":selected").attr("data-service_id") != "all" && $(e.target).find(":selected").attr("data-service_id") != "cheapest") {
                ...
                $("#courier_delivery_days").val($("#courier_services option:selected").attr("data-courier_info"));
                ...
                if ($("#courier_services option:selected").attr("data-dropoff") == "yes") {
                    var list = '<option value="optional">Drop Off Point</option>';
        ...
        function doSetupCourierCheck(data) {
            const serviceData = $("#courier_services option:selected")
            var courier_service = serviceData.attr("data-service_id");
            var service_name = serviceData.attr("data-service_name");
            var courier_id = serviceData.attr("data-courier_id");
            var courier_name = serviceData.attr("data-courier_name");
            var courier_logo = serviceData.attr("data-courier_logo");
            var courier_info = $('#courier_delivery_days').val() !== '' ? $('#courier_delivery_days').val() : serviceData.attr("data-courier_info");
            var dropoff_option = serviceData.attr("data-dropoff");
            var courier_display_name = $("input#courier_display_name").val();
            var courier_sample_cost = serviceData.attr("data-sample_cost");
            var charges_option = $("select#charges_option option:selected").val();
            var charges_value = 0;
            var dropoff_point = "";
            var dropoff_name = '';
            ...
            var free_shipping_option = 0; // make default no freeshipping
            ...
            if (dropoff_option == "yes") {
                if ($('#dropoff_point').children('.selected').length === 0) {
                    $($('#dropoff_point').children()[0]).addClass('selected').attr('selected', 'selected').siblings().removeClass('selected').removeClass('selected')
                }
                dropoff_point = $("select#dropoff_point option:selected").val();
                dropoff_name = $("select#dropoff_point option:selected").data('dropoff_name');
            }
            //perform validation
            if (err_msg != "") {
                alert(err_msg);
                ...
                var data = {
                    ...
                    courier_dropoff_name: dropoff_name,
