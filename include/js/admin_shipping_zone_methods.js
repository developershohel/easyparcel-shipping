    ...
    $(function () {
        var $table = $('.wc-shipping-zone-methods'),
            ...
            ShippingMethod = Backbone.Model.extend({
                ...
                save: function () {
                    $.post(
                        ajaxurl + (ajaxurl.indexOf('?') > 0 ? '&' : '?') + 'action=easyparcel_shipping_zone_methods_save_changes',
                        {
            ...
            ShippingMethodView = Backbone.View.extend({
                ...
                onAddShippingMethod: function (event) {
                    ...
                    if (!jQuery.isEmptyObject(changes)) {
                        $.post(ajaxurl + (ajaxurl.indexOf('?') > 0 ? '&' : '?') + 'action=easyparcel_shipping_zone_methods_save_changes',
                            {
                                wc_shipping_zones_nonce: data.wc_shipping_zones_nonce,
                                changes: changes,
                                zone_id: data.zone_id
