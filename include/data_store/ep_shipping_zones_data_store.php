<?php
/**
 * Class EP_Shipping_Zone_Data_Store file.
 *
...
require_once 'interface/ep_object_data_store_interface.php';
require_once 'interface/ep_shipping_zone_data_store_interface.php';
 ...
 * @version  3.0.0
 */
class EP_Shipping_Zone_Data_Store extends WC_Data_Store_WP implements EP_Shipping_Zone_Data_Store_Interface, EP_Object_Data_Store_Interface {
	...
	public function read( &$zone ) {
		...
		$zone_data = $wpdb->get_row(
			$wpdb->prepare(
				"SELECT zone_name, zone_order FROM {$wpdb->prefix}easyparcel_zones WHERE zone_id = %d LIMIT 1",
				$zone->get_id()
			)
		);

		if ( ! $zone_data ) {
			throw new Exception( esc_html_e( 'Invalid data store.', 'easyparcel-shipping' ) );
		}
	...
	public function delete( &$zone, $args = array() ) {
		...
		if ( $zone_id ) {
			...
			// Delete zone.
			$wpdb->delete( $wpdb->prefix . 'easyparcel_zone_locations', array( 'zone_id' => $zone_id ) );
			$wpdb->delete( $wpdb->prefix . 'easyparcel_zones', array( 'zone_id' => $zone_id ) );
	...
	public function get_zone_id_from_package( $package ) {
		...
		$postcode_locations = $wpdb->get_results( "SELECT zone_id, location_code FROM {$wpdb->prefix}easyparcel_zone_locations WHERE location_type = 'postcode';" );
		...
		// Get matching zones.
		return $wpdb->get_var(
			"SELECT zones.zone_id FROM {$wpdb->prefix}easyparcel_zones as zones
			LEFT OUTER JOIN {$wpdb->prefix}easyparcel_zone_locations as locations ON zones.zone_id = locations.zone_id AND location_type != 'postcode'
	...
	public function get_zones() {
		global $wpdb;
		return $wpdb->get_results( "SELECT zone_id, zone_name, zone_order FROM {$wpdb->prefix}easyparcel_zones order by zone_order ASC, zone_id ASC;" );
	}
	...
	private function read_zone_locations( &$zone ) {
		...
		$locations = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT location_code, location_type FROM {$wpdb->prefix}easyparcel_zone_locations WHERE zone_id = %d",
				$zone->get_id()
			)
		);
	...
	private function save_locations( &$zone ) {
		...
		global $wpdb;
		$wpdb->delete( $wpdb->prefix . 'easyparcel_zone_locations', array( 'zone_id' => $zone->get_id() ) );
