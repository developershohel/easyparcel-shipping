...
class EP_Shipping_Zones {
    ...
    public static function get_zones($context = 'admin') {
        ...
        foreach ($raw_zones as $raw_zone) {
            ...
            $zones[$zone->get_id()] = $zone_data;
            $zones[$zone->get_id()]['zone_id'] = $zone->get_id();
            $zones[$zone->get_id()]['couriers'] = $couriers;
    ...
    public static function ep_get_zones() {
        global $wpdb;
        return $wpdb->get_results("SELECT * FROM `" . $wpdb->prefix . "easyparcel_zones`");
    }
    ...
    public static function get_zone_courier($zone_id = 0) {
        ...
        return $wpdb->get_results(
            $wpdb->prepare(
                "SELECT * FROM `" . $wpdb->prefix . "easyparcel_zones_courier` WHERE zone_id = %d ORDER BY courier_order ASC",
                absint($zone_id)
            )
        );
    }
    ...
    public static function ep_get_valid_zone_courier($destination) {
        global $wpdb;
        $raw_zones = $wpdb->get_results("SELECT * FROM `" . $wpdb->prefix . "easyparcel_zones` JOIN `" . $wpdb->prefix . "easyparcel_zones_courier` ON `" . $wpdb->prefix . "easyparcel_zones`.zone_id = `" . $wpdb->prefix . "easyparcel_zones_courier`.zone_id");
